...
async function get_token(){
    ...
    return axios
        .post("https://tecweb-js.insper-comp.com.br/token", { username: "danielmd3" }, config)
...
async function solve_and_submit_exercises(token, exercises) {
  for (const exerciseSlug in exercises) {
      ...
      if(exercise.titulo === "Soma valores"){
        answer = exercise.entrada.a + exercise.entrada.b; // Calcula a soma entre a e b

      }
      else if(exercise.titulo === "Tamanho da string"){
        answer = exercise.entrada.string.length; // Calcula o tamanho da string
      }
      else if(exercise.titulo === "Nome do usuário"){
        entrada = exercise.entrada.email;
        answer = entrada.substring(0, entrada.indexOf('@')); // Pega o nome do usuário
      }
      ...
      let lista_entrada = exercise.entrada.endpoints;
      let soma = 0;
      const config2 = {
        headers: {
            "Content-Type": "application/json",
    ...
    while (true){
      if(i === 0){    
      response = await axios.get(dic["inicio"], config2);
      }
